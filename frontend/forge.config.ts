...
const config: ForgeConfig = {
  packagerConfig: {
    asar: true,
    extraResource: [
      path.join(projectRoot, "mcp_tools"),
      path.join(projectRoot, "core"),
      path.join(projectRoot, "utils"),
      path.join(projectRoot, "mobile_mcp"),
      path.join(projectRoot, "config.py"),
      path.join(projectRoot, "requirements.txt"),
      path.join(projectRoot, "__init__.py"),
      // Python 虚拟环境（由 scripts/bundle-python.sh 构建）
      path.join(projectRoot, ".venv_bundle"),
    ],
  },
  rebuildConfig: {},
  makers: [
